...
(function ($) {
	...
	// ScrollSmoother-compatible sticky header
	if ($(".header-sticky").length) {
		ScrollTrigger.create({
			start: "top-=100 top",
			onEnter: () => $(".header-sticky").addClass("sticky"),
			onLeaveBack: () => $(".header-sticky").removeClass("sticky"),
		});
	}
	...
	function tjBaackTopController() {
		...
		if (!scrollElementWrap.length) return;

		const updateScrollUI = () => {
			const smoother = (typeof ScrollSmoother !== "undefined") ? ScrollSmoother.get() : null;
			const scrollTopPos = smoother ? smoother.scrollTop() : (document.documentElement.scrollTop || window.pageYOffset);
			if (scrollTopPos > 100) {
		...
		ScrollTrigger.addEventListener("scroll", updateScrollUI);
